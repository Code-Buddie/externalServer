...
require "./config.php";
$sql = "SELECT email, name, visits, visitTimes FROM `returningUsers` GROUP BY email";
try {
...
while ($value = $stmt->fetch(PDO::FETCH_NUM, PDO::FETCH_ORI_NEXT)) {
    echo "<tr><td>" . $count . "</td><td>" . $value[1] . "</td><td>" . $value[0] . "</td><td>" . $value[2] . "</td><td>".$visits=explode(',', $value[3]); foreach($visits as $visit){
        echo '<ul>'.$visit.'</ul>';  
    } "</td> </tr>";
    $count++;
}
